...
std::unique_ptr<SceneGraph> SceneGraph::sceneGraphInstace = nullptr;
...
{
	if (sceneGraphInstace.get() == nullptr) {
		sceneGraphInstace.reset(new SceneGraph);
	}
	return sceneGraphInstace.get();
}
...
void SceneGraph::AddGameObject(GameObject* gameObject_, std::string tag_)
{
	if (tag_ == "") {
		...
		Debugger::Error("Trying to add a GameObject with a tag" + tag_ + " that already exists", "SceneGraph.cpp", __LINE__);
		std::string newTag = "GameObject" + std::to_string(sceneGameObjects.size() + 1);
		gameObject_->SetTag(newTag);
		sceneGameObjects[newTag] = gameObject_;
	}
